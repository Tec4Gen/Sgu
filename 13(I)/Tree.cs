...
namespace Example
{
    public class BinaryTree //класс, реализующий АТД «дерево бинарного поиска»
    {
        ...
        private class Node
        {
            ...
            {   
                Count++; //Считаем сколько у нас всего 
                inf = nodeInf;
                middle +=inf;
                left = null;
                rigth = null;
            ...
            #region Мой велосипед
            public static void MiddleArgument(Node r) //Ищем среднее
            {
                if (Count!=0) 
                {
                    middle /= Count;
            ...
            public static void MyPreorder(Node r) //прямой обход дерева c поиском всех узлов меньше среднего арифметического
            {
                if (r != null)
                {
                    if (r.inf < middle)
        ...
        public void FindNode()
        {
            if (tree!=null) 
            {
                Node.MiddleArgument(tree);
                Console.WriteLine($"Количество узлов которые меньше среднего арифметического {Node.middle} = {Node.CountFind}");
            }
        }
    }
}
