...
namespace Example
{
    public class BinaryTree //класс, реализующий АТД «дерево бинарного поиска со счетчиком вершин в дереве»
    {
        ...
        private class Node
        {
            private static List<List<int>> Levels = new List<List<int>>();
            ...
            public static void Preorder(Node r) //прямой обход дерева
            {
                if (r != null)
                {
                    ...
                    Preorder(r.rigth);
                    NowLevel--;
                    Levels[NowLevel].Add((int)r.inf);

                }
            }


            public static void ShowLevels(Node r)  //симметричный обход дерева
            {
                if (r != null)
                {
                    int n = 0;
                    foreach (var First in Levels)
                    {
                        Console.Write("Уровень {0}: ",n++);
        ...
        public void ShowLelvels()
        {
            Node.ShowLevels(tree);
        }
